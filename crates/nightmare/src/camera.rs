pub fn camera_system(context: &mut crate::app::Context) {
    ...
    let speed = 10.0 * context.delta_time.as_secs_f32();
    ...
    orientation.zoom(6.0 * context.io.mouse.wheel_delta.y * (context.delta_time.as_secs_f32()));

    if context.io.mouse.is_middle_clicked {
        orientation.pan(&(context.io.mouse.position_delta * context.delta_time.as_secs_f32()));
        sync_transform = true;
    }

    if context.io.mouse.is_right_clicked {
        let mut delta = context.io.mouse.position_delta * context.delta_time.as_secs_f32();
        delta.x *= -1.0;
        delta.y *= -1.0;
